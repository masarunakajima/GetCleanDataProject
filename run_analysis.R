...
# read the train and test files (4. get variable names)
x_train <- read.table("UCI HAR Dataset/train/X_train.txt", col.names = features_vec)
y_train <- read.table("UCI HAR Dataset/train/y_train.txt", col.names = c("Activity"))
x_test <- read.table("UCI HAR Dataset/test/X_test.txt", col.names = features_vec)
